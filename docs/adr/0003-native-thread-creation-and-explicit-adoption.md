...
This exception is deliberately non-atomic and local. Executor selection routes the command to the intended physical host; the command does not attest local process argv, invoke an Amp server tool, register a project, dispatch remotely, or fall back to an Orb. In particular, a plain local `amp --no-tui` runner is valid and its optional `--runner-id` argv alias is not an amux spawn input. The retained outcome preserves and reports the exact thread/window/pane, distinguishes the local input-attempt result from every completed persistence phase and the unavailable delivery acknowledgement, and warns that unsent prompt bytes may remain in the composer. The worker row's optional assignment-state field durably preserves this uncertainty; legacy/adopted rows remain `unknown`. List and doctor report assignment state separately from local pane state. This is ownership metadata, not an operation or prompt ledger. There is no automatic retry, repaste, submit, transcript/pane/history/preview read, alternate receiver, reconciliation, search, archive, kill, or cleanup. Owner-directed authenticated follow-up is manual recovery outside this contract. The original decision and migration evidence below remain the historical and preferred architecture for repositories supported by native creation.
