...
The owner subsequently accepted one narrow exception to the preferred native-create/adopt architecture. A lean local `amux spawn` path may serve a repository that is not an Amp Workspace Project on an explicitly selected physical executor. It canonicalizes the target cwd, invokes `amp threads new --mode <exact-mode>` once with that cwd, creates one exact tmux `amp threads continue <thread>` worker, attempts one bounded prompt paste, and attempts one Enter only after paste success. The public local Amp CLI exposes no compatible delivery acknowledgement, so successful or failed tmux input commands establish only a bounded input-attempt result. Without a second input attempt, the command persists exact worker ownership with assignment state `retained_indeterminate` plus optional existing-group ownership, then returns exit `1` as `retained-indeterminate`; no persistence phase or live local client means that the assignment was delivered or is executing. `amux worker spawn` is an exact alias, not another implementation.

This exception is deliberately non-atomic and local. Executor selection routes the command to the intended physical host; the command does not attest local process argv, invoke an Amp server tool, register a project, dispatch remotely, or fall back to an Orb. In particular, a plain local `amp --no-tui` runner is valid and its optional `--runner-id` argv alias is not an amux spawn input. The retained outcome preserves and reports the exact thread/window/pane, distinguishes the local input-attempt result from every completed persistence phase, reports delivery as indeterminate and acknowledgement as unavailable, and warns that unsent prompt bytes may remain in the composer. Delivery and execution are unproven. The worker row's optional assignment-state field durably preserves this uncertainty; legacy/adopted rows remain `unknown`. List and doctor report assignment state separately from local pane state, including human doctor output. This is ownership metadata, not an operation or prompt ledger. There is no automatic retry, repaste, submit, transcript/pane/history/preview read, alternate receiver, reconciliation, search, archive, kill, or cleanup. Owner-directed authenticated follow-up is manual recovery outside this contract. The original decision and migration evidence below remain the historical and preferred architecture for repositories supported by native creation.
