...
Native creation owns execution placement. Adoption does not re-home, migrate, or retarget the thread's native executor, and a local continued-thread pane is not evidence that future turns run in that pane's workdir. Create the thread directly on the executor and physical workdir where its future turns must run. In particular, Orb creation followed by physical adoption is never an execution-migration mechanism.
