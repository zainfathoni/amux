...
- Bulk operations preflight the complete plan, then continue independent actions after runtime failures. Runner restart is the containment exception: after one replacement fails, later runner restarts in that operation are skipped so a shared launch defect cannot stop the remaining healthy fleet; independent non-runner actions may still continue. Exit `0` means no failures, `1` means partial or complete runtime failure, and `2` means request or preflight rejection before mutation.
...
- Missing worker and runner workdirs are repaired explicitly through mode-specific reconciliation; launch never silently mutates configuration. Worker reconciliation canonicalizes registry workdir identity, removes a worker binding only on proven absence with no local runtime or blocked never-authorized report, and preserves unregistered directories because `workers.tsv` is the sole thread↔workdir authority. Unreadable, non-directory, relative, and unknown worker workdir states fail closed.
