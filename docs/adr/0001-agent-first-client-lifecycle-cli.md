...
> **Current scope:** [ADR 0008](0008-retain-machine-local-runner-host-and-drain-coordination.md) retains this ADR's machine-local runner registry, launch, maintenance, diagnostics, and fail-closed lifecycle contracts. [ADR 0009](0009-remove-active-legacy-coordination-surfaces.md) removes the worker and core coordination routes described below and leaves their historical stores inert. Provider-specific evidence remains under separate explicit gates; native-created work does not acquire an Amux lifecycle.
...
- Bulk operations preflight the complete plan, then continue independent actions after runtime failures. Reconcile treats stale-registration removal as an all-or-nothing sub-plan: any refused removal candidate suppresses every worker and aggregate runner removal in that sub-plan, while independent shelf/archive synchronization for present workers remains available and makes a mixed real invocation a partial runtime failure. Runner restart is the containment exception: after one replacement fails, later runner restarts in that operation are skipped so a shared launch defect cannot stop the remaining healthy fleet; independent non-runner actions may still continue. Exit `0` means no failures, `1` means partial or complete runtime failure, and `2` means request or preflight rejection before mutation.
...
- Missing worker and runner workdirs are repaired explicitly through mode-specific reconciliation; launch never silently mutates configuration. Worker reconciliation canonicalizes registry workdir identity for comparison and evidence while preserving the `workers.tsv` spelling in worker output, removes a worker binding only on proven absence with no local runtime or blocked never-authorized report, and preserves unregistered directories because `workers.tsv` is the sole thread↔workdir authority. Report and runtime inspection are removal-only dependencies; present workers retain legacy shelf/archive synchronization. Unreadable, non-directory, relative, and unknown worker workdir states fail closed.
...
- `reconcile` is an aggregate lifecycle command with mode-specific worker and runner implementations. It accepts current, canonical resource, workspace, or explicit machine-wide scope; canonical workdir scope checks both modes, and workspace reconciliation jointly preflights both modes.
- Runner reconciliation diagnoses conflicting Amp-owned PID markers but never deletes them without a supported conditional ownership protocol. Canonical runner launch verifies that the exact created pane's retained shell survives with one exact direct `amp --no-tui` child; legacy `exec` panes verify Amp directly.
...
- The bundled `/amux` skill is versioned as part of the product surface. Normal global installation uses the `skills` CLI from the public repository; machines that deliberately maintain a clean local `main` checkout may opt into installer-managed absolute links for all four bundled skills. The installer preserves replaced installations and never updates the source checkout implicitly.
