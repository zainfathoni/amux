...
- The bundled `/amux` skill is versioned as part of the product surface. Normal global installation uses the `skills` CLI from the public repository; machines that deliberately maintain a clean local `main` checkout may opt into installer-managed absolute links for all three bundled skills. The installer preserves replaced installations and never updates the source checkout implicitly.
