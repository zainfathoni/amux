...
Provider execution is outside Amux's maintained lifecycle and recovery core. Tycho may own machine and provider routing for Claude Code and Pi/Codex Spark. `/amux-tycho` is an experimental, practically usable, explicit-request-only semantic-report bridge: the immutable real Amp origin retains coordinator, consume, and acknowledgement authority, while Tycho has report-only authority. `/amux-claude` and `/amux-pi` remain experimental, explicit-request-only fallback and reference paths. None of these routes widens core Amux into provider-policy machinery. Forgex remains experimental and orthogonal, not an Amux replacement.
