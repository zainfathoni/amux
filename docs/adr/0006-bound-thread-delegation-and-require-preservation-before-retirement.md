...
- Provider execution remains outside maintained Amux core. Tycho may own machine and provider routing for Claude Code and Pi/Codex Spark. A later design may define a provider-neutral Amp↔Tycho package, but this ADR selects no routing authority, package schema, transport, or stable Tycho/provider contract. `/amux-claude` and `/amux-pi` remain experimental, explicit-request-only fallback and reference paths. Forgex remains experimental and orthogonal, not an Amux replacement.
