...
| Versions and capability diagnostics | Darwin; Claude Code 2.1.214; tmux 3.7b; experimental protocol version 1. Initial interactive input and semantic submission were supported. Automatic interactive input and a session-hook contract were unavailable. Managed-policy runtime effect, strict-MCP runtime behavior, and read-confinement runtime remained explicitly `untested`; policy confinement was not treated as an OS sandbox. |
