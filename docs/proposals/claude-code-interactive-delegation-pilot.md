...
### Optional follow-on requiring separate approval

After both local checkpoints, a separately approved third pilot may exercise managed teleport or explicit adoption. Its design is retained below so local receipt and identity choices do not preclude it, but approval to implement Pilots 1 and 2 does not authorize teleport code or adoption behavior.
...
### Mutable routing and delivery

- current target: an Amp thread or the machine-local inbox;
- pilot notification transport: a verified local Amp pane wake-up when available;
- recovery action: `/amux` consumption from the local inbox;
- routing-change events that retain the previous target and provenance.

The target, attempted transport, and recovery action are distinct. None is part of the immutable delegation binding.

A supported cross-thread append may become a future transport, but it is not part of Pilots 1 or 2.
...
- session acquired, Claude client waiting, or acquisition failed;
- `input_request`, correlated response, and Claude-side response acceptance or superseding state;
- semantic report submission and validation;
- validation failure;
...
Each event has a stable identity. Exact replay produces a duplicate outcome; conflicting reuse is rejected. Events are append-only within the pilot receipt, transitions are validated, and report lifecycle follows `valid → delivered → acknowledged → parked`: delivery requires a valid report, acknowledgement requires delivery, and park requires the corresponding acknowledgement. Notification is only a wake-up attempt and never establishes delivery. A materialized state may be derived for inspection, but it must preserve source status and provenance.

Receipt acknowledgement records that a report was consumed and permits parking; it is transport state, not a superseding source event. An `input_request` remains semantically unresolved until the Claude side confirms that the correlated response was accepted or emits another explicit superseding event. Coordinator delivery or acknowledgement alone may mark the request seen, never resolved.
...
6. A correlated supported session-start signal records session acquisition and local session ID. A waiting signal is factual Claude client state, not proof that the composer is safe for injection; an unverified prompt leaves acquisition pending.
7. Claude may submit a durable `input_request`. The coordinator's answer consumes the bounded follow-up budget but is handled through manual interaction by default. A correlated Claude-side acceptance or later superseding semantic event resolves the request. Automatic tmux response delivery remains capability-gated and unavailable unless a stronger vendor contract proves exact safe-input state.
...
10. The coordinator consumes the valid report, recording delivery, then separately acknowledges it. Claude remains active until acknowledgement.
...
2. Before teleport, the skill verifies repository identity, takes exclusive edit ownership of the dedicated worktree path and checked-out branch slot, and reserves lifecycle ownership of the resulting local Claude client and dedicated tmux pane. It blocks if the cloud branch is already owned by another worktree or another actor may edit the target. It never offers or accepts an automatic stash.
...
### Optional Pilot 3: teleport or adoption, separately approved

Only after both local checkpoints and separate explicit approval, exercise managed teleport or explicit adoption on real work. A mutating adopted session qualifies only when `/amux` can verify repository identity, establish a clean acquisition baseline, take exclusive edit ownership of the worktree and checked-out branch slot, and own the local Claude session and pane lifecycle. Otherwise it remains active but unmanaged or read-only and is recorded as untested for mutation.
...
Unresolved or recoverable receipts do not expire. Acknowledged and parked raw receipts become cleanup-eligible after 30 days. Only curated aggregates belong in a later proposal by default. Any representative artifact or Amp thread reference requires explicit owner selection plus a privacy and access review before it is committed to a public repository.
...
- Treat a successful tmux wake-up sent to a verified Amp pane as notification only, never acknowledgement.
...
- the pilot cannot remain useful without unsafe input automation or output scraping;
