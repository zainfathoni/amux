...
| PR | Disposition |
| --- | --- |
| [#360](https://github.com/zainfathoni/amux/pull/360) | Merged one-time read-only inventory. Do **not** run it or delete its surfaces now. Sunset activates only after owner acceptance or explicit incomplete/error disposition **plus** no-repeat confirmation; only then delete before the next release. |
