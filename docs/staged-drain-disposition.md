...
This ledger applies [ADR 0009](adr/0009-remove-active-legacy-coordination-surfaces.md) and [ADR 0011](adr/0011-consolidate-on-native-multi-directory-runners.md) to the still-valid boundaries in [ADR 0007](adr/0007-retire-amux-through-native-cutover-and-staged-drain.md). Amux is not being fully deprecated or archived. Native Amp owns task coordination, multi-directory runner execution, and automatic updates. Amux retains declarative runner profiles and systemd/launchd service installation; core legacy coordination commands are removed and their stores are inert.
...
| OS activation | systemd/launchd executes the resolved Amp binary directly and keeps it alive. Amux is not resident. Native Amp owns automatic updates and idle restart. |
