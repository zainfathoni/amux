...
- Amux worker, spawn/adoption, group, report, callback, deadline, shelf, worker-teardown, and finish routes are removed. Never revive them through an older binary, direct store edits, or synthetic compatibility state. `amux runner teardown` is a separate machine-local worktree operation and never mutates those stores or native thread state.
