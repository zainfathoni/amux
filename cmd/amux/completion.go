...
var completionCommands = []completionCommand{
	...
	{Name: "remove", Description: "Remove worker configuration; configured runner deletion fails closed", Flags: []string{"--workspace", "--thread", "--workdir", "--current", "--all", "-w", "-t", "-d"}},
	{Name: "doctor", Description: "Diagnose worker and runner state", Flags: []string{"--workspace", "--thread", "--workdir", "--current", "--all", "-w", "-t", "-d"}},
	{Name: "reconcile", Description: "Repair worker drift; retain missing configured runner rows", Flags: []string{"--workspace", "--thread", "--workdir", "--current", "--all", "-w", "-t", "-d"}},
	{
		Name:        "worker",
		...
		Subcommands: []completionCommand{
			...
			{Name: "adopt", Description: "Continue an exact persisted pre-cutover drain-eligible adoption operation", Flags: []string{"--workspace", "--window", "--workdir", "--thread", "--group", "-w", "-W", "-d", "-t"}},
	...
	},
	{
		...
		Subcommands: []completionCommand{
			...
			{Name: "remove", Description: "Retain configured runners and fail closed", Flags: []string{"--workspace", "--workdir", "--current", "--all", "-w", "-d"}},
			{Name: "doctor", Description: "Diagnose runners", Flags: []string{"--workspace", "--workdir", "--current", "--all", "-w", "-d"}},
			{Name: "reconcile", Description: "Keep present rows unchanged; retain missing rows and fail closed", Flags: []string{"--workspace", "--workdir", "--current", "--all", "-w", "-d"}},
		},
	},
    ...
    case $command in
      worker)
        if [[ -z $leaf ]]; then
          ...
          case $leaf in
            adopt) _arguments '--workspace[workspace]:workspace:' '--window[window]:window:' '--workdir[working directory]:directory:_directories' '--thread[exact persisted pre-cutover drain thread id or URL]:thread:' '--group[exact persisted pre-cutover group only]:group:' '-w[workspace]:workspace:' '-W[window]:window:' '-d[working directory]:directory:_directories' '-t[thread id or URL]:thread:' ;;
