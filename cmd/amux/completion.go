...
var completionCommands = []completionCommand{
	...
	{Name: "runner", Description: "Manage machine-local workdir-bound runners", Subcommands: []completionCommand{
		...
		{Name: "service", Description: "Manage native Amp runner services", Subcommands: []completionCommand{
			{Name: "install", Description: "Install configured runner services"},
			{Name: "remove", Description: "Remove installed runner services"},
			{Name: "doctor", Description: "Diagnose configured runner services"},
		}},
		{Name: "list", Description: "List configured runners", Flags: runnerLifecycleFlags},
...
_amux_complete() {
  ...
  for ((i=1; i<COMP_CWORD; i++)); do
    ...
    elif [[ "$command" == runner && ( "$leaf" == maintenance || "$leaf" == service ) && -z "$branch" ]]; then branch="$word"
    fi
  done
  ...
  case "$command" in
    runner)
      ...
      elif [[ "$leaf" == service && -z "$branch" ]]; then COMPREPLY=( $(compgen -W "%s" -- "$cur") )
      ...
      elif [[ "$leaf" == maintenance || "$leaf" == service ]]; then COMPREPLY=()
...
`, strings.Join(commandNames(completionCommands), " "), strings.Join(globalCompletionFlags, " "), strings.Join(commandNames(runnerCompletionCommands()), " "), strings.Join(commandNames(runnerMaintenanceCompletionCommands()), " "), strings.Join(commandNames(runnerServiceCompletionCommands()), " "))
}

func writeZshCompletion(w io.Writer) {
	fmt.Fprint(w, "#compdef amux\n# zsh completion for amux; generated by amux completion zsh\n\n")
	fmt.Fprintln(w, "local context state line command leaf branch word\nlocal -i i\nlocal -a commands runner_commands maintenance_commands service_commands")
	...
	fmt.Fprintln(w, ")\nservice_commands=(")
	for _, command := range runnerServiceCompletionCommands() {
		fmt.Fprintf(w, "  %q\n", command.Name+":"+command.Description)
	}
	fmt.Fprintln(w, `)
  ...
  args)
    case $command in
      runner)
        ...
        elif [[ $leaf == service && -z $branch ]]; then _describe -t commands 'service command' service_commands
        ...
        elif [[ $leaf == maintenance || $leaf == service ]]; then return
...
func writeFishCompletion(w io.Writer) {
	...
	for _, command := range runnerServiceCompletionCommands() {
		fmt.Fprintf(w, "complete -c amux -n '__fish_seen_subcommand_from service; and not __fish_seen_subcommand_from install remove doctor' -a %s -d %s\n", fishQuote(command.Name), fishQuote(command.Description))
	}
	...
	fmt.Fprintln(w, "complete -c amux -n '__fish_seen_subcommand_from list launch park restart remove doctor reconcile pin; and not __fish_seen_subcommand_from maintenance service' -l workspace -s w -r")
	fmt.Fprintln(w, "complete -c amux -n '__fish_seen_subcommand_from list launch park restart remove doctor reconcile pin unpin teardown; and not __fish_seen_subcommand_from maintenance service' -l workdir -s d -r")
	fmt.Fprintln(w, "complete -c amux -n '__fish_seen_subcommand_from list launch park restart remove doctor reconcile pin unpin teardown; and not __fish_seen_subcommand_from maintenance service' -l current-dir -s c")
	fmt.Fprintln(w, "complete -c amux -n '__fish_seen_subcommand_from list launch park restart remove doctor reconcile; and not __fish_seen_subcommand_from maintenance service' -l all")
	fmt.Fprintln(w, "complete -c amux -n '__fish_seen_subcommand_from list launch park restart remove doctor reconcile pin unpin; and not __fish_seen_subcommand_from maintenance service' -l current")
	fmt.Fprintln(w, "complete -c amux -n '__fish_seen_subcommand_from pin; and not __fish_seen_subcommand_from maintenance service' -l runner-id -s i -r")
	fmt.Fprintln(w, "complete -c amux -n '__fish_seen_subcommand_from pin; and not __fish_seen_subcommand_from maintenance service' -l restart")
	fmt.Fprintln(w, "complete -c amux -n '__fish_seen_subcommand_from teardown; and not __fish_seen_subcommand_from maintenance service' -l confirm-plan -r")
...
func runnerServiceCompletionCommands() []completionCommand {
	for _, command := range runnerCompletionCommands() {
