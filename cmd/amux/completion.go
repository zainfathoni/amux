...
var completionCommands = []completionCommand{
	...
	{Name: "reconcile", Description: "Explicitly repair worker and runner drift", Flags: []string{"--workspace", "--thread", "--workdir", "--current", "--all", "-w", "-t", "-d"}},
	{
		...
		Subcommands: []completionCommand{
			...
			{Name: "spawn", Description: "Create one projectless local physical-host worker", Flags: []string{"--runner-id", "--workdir", "--workspace", "--window", "--group", "--mode", "--prompt-file", "-d", "-w", "-W", "-m"}},
    ...
    worker)
      if [[ -z "$leaf" ]]; then
        ...
        case "$leaf" in
          ...
          spawn) COMPREPLY=( $(compgen -W "--runner-id --workdir --workspace --window --group --mode --prompt-file -d -w -W -m" -- "$cur") ) ;;
  ...
  args)
    case $command in
      worker)
        if [[ -z $leaf ]]; then
          ...
          case $leaf in
            ...
            spawn) _arguments '--runner-id[exact local Amp runner ID]:runner:' '--workdir[canonical physical workdir]:directory:_directories' '--workspace[tmux workspace]:workspace:' '--window[semantic window]:window:' '--group[optional existing group]:group:' '--mode[exact mode]:mode:(low medium high ultra)' '--prompt-file[prompt path or -]:file:_files' '-d[canonical physical workdir]:directory:_directories' '-w[tmux workspace]:workspace:' '-W[semantic window]:window:' '-m[exact mode]:mode:(low medium high ultra)' ;;
      ...
      unshelve)
        ...
        ;;
      spawn)
        _arguments '--runner-id[exact local Amp runner ID]:runner:' '--workdir[canonical physical workdir]:directory:_directories' '--workspace[tmux workspace]:workspace:' '--window[semantic window]:window:' '--group[optional existing group]:group:' '--mode[exact mode]:mode:(low medium high ultra)' '--prompt-file[prompt path or -]:file:_files' '-d[canonical physical workdir]:directory:_directories' '-w[tmux workspace]:workspace:' '-W[semantic window]:window:' '-m[exact mode]:mode:(low medium high ultra)'
        ;;
      teardown)
        _arguments '--thread[select by thread id or URL]:thread:' '--workspace[select workspace]:workspace:' '--current[current worker]' '--all[all workers]' '-t[select by thread id or URL]:thread:' '-w[select workspace]:workspace:'
        ;;
