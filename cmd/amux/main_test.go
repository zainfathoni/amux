	...
	tests := []struct {
		...
		},
		{
			...
			want: []string{
				...
				"\"spawn:Removed; create a native Amp thread, then use worker adopt\"",
		...
		},
		{
			...
			want: []string{
				...
				"complete -c amux -f -n '__fish_use_subcommand' -a 'spawn' -d 'Removed; create a native Amp thread, then use worker adopt'",
				...
				"complete -c amux -f -n 'test (__fish_amux_root_command) = worker; and test -z (__fish_amux_worker_leaf)' -a 'spawn' -d 'Removed; create a native Amp thread, then use worker adopt'",
