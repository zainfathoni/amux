	...
	tests := []struct {
		...
		},
		{
			...
			want: []string{
				...
				"\"spawn:Retain one projectless local worker with indeterminate delivery\"",
		...
		},
		{
			...
			want: []string{
				...
				"complete -c amux -f -n '__fish_use_subcommand' -a 'spawn' -d 'Retain one projectless local worker with indeterminate delivery'",
				...
				"complete -c amux -f -n 'test (__fish_amux_root_command) = worker; and test -z (__fish_amux_worker_leaf)' -a 'spawn' -d 'Retain one projectless local worker with indeterminate delivery'",
