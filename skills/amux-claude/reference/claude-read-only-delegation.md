...
To use an approved explicit model, include exact `"model":"claude-fable-5"`, `"model":"claude-opus-5"`, or `"model":"claude-opus-4-8"` in this request and in the otherwise identical `launch plan` and `launch execute` requests. This selection requires explicit owner authorization for that exact value. Omit `model` everywhere to retain Claude's existing default-model argv exactly; omission does not select or attest to any particular default. The helper accepts only these exact approved identifiers and never chooses, normalizes, aliases, falls back, or retries a model autonomously. Exact `claude-opus-5` support applies only to this local read-only route; it does not broaden the mutating or fresh-Orb contracts.
