# Experimental Claude delegation recovery

Use only the branch matching the failed workflow criterion. Preserve the receipt, private state, process, and worktree unless an explicit owner action says otherwise.

## Receipt replay or conflict

After an interrupted helper mutation, inspect the receipt and retry the identical operation with the **same event ID** and exact payload. `duplicate` is success. A conflict means that identity already denotes different content: stop and investigate; never erase, rewrite, migrate, or replace historical evidence to force progress.

## Launch or acquisition mismatch

Stop before receipt creation when a read-only request omits `expected_launch_policy_digest` or the helper rejects it as malformed or different from the selected policy. Recompute neither value by hand and do not rewrite or inject content after planning. Stop before launch when repository, base, clean linked-worktree status, private packet mode, policy digest, or required capability differs. If durable `launch_intent` lacks `launch_completed`, the outcome is indeterminate: do not retry launch, guess, or create another window. Inspect tmux and the receipt manually; acquire only one exact Claude session incarnation or leave the receipt recoverable.

## Missing or invalid semantic message

Pane prose, idle state, stop hooks, and process exit are not reports. Do not infer completion or blockage. Keep the receipt unresolved and inspect only bounded structural diagnostics. Never preserve a pane capture or transcript as a substitute report.

## Input request

Consume the request to mark it seen without resolving it. Handle the response manually and within the declared follow-up bound. Do not automatically inject text into Claude's composer. Record acceptance only after explicit correlated Claude-side confirmation; otherwise leave the request unresolved or let a later explicit source event supersede it.

## Notification unavailable or failed

Do not search for a convenient Amp pane or weaken identity checks. Recover the report through `receipt show` and `inbox consume`; leave the receipt recoverable until that succeeds. Exact replay of an attempt missing its result records `unavailable` without resending. A wake-up token never establishes delivery.

## Delivery, acknowledgement, or parking rejection

Consume the current valid report before acknowledging that same message, and resolve or explicitly supersede any input request. Keep Claude active until acknowledgement. If exact incarnation re-verification fails, do not kill a pane by name, PID, workdir, or session ID alone. Preserve the acknowledged receipt and inspect whether tmux, process, workdir, launch command, or incarnation changed. If `park_intent` is durable without a result, retry the same event with `"recover":true`; only exact matching identity or confirmed exact-pane absence may complete it. Do not automatically park or clean up.

## Paired worker teardown blocked

For the exact `receipt_store_invalid_or_unavailable` blocker, do not repair provider evidence. With explicit owner authorization, the provider-only quarantine route may plan exact `park`, then apply the byte-identical plan with its bound authorization. Supply only an explicitly known registered-store SHA-256 selector and an explicit bounded artifact manifest; never discover ownership from paths or process metadata. Any changed or unavailable object blocks. Interrupted apply is unresolved: inspect its operation hash and do not retry the external park. Quarantine preserves all evidence and does not permit teardown, removal, archive, detach, cleanup, retirement, or no-pair classification.

Treat every paired worker teardown blocker as authoritative: preserve the Amp worker, Claude pane, private artifacts/worktree, receipt, report, group history, canonical lifecycle registry, and durable origin fence. Use the returned non-content blocker code to inspect the existing branch above: consume and acknowledge reports deliberately, resolve input, repair an unavailable owner-private registered store, or recover the exact durable park intent. Never adopt a legacy or unrelated pane, rewrite the origin binding, issue a new launch after indeterminate intent, or infer ownership from names, cwd, PID, issue number, tmux placement, or Claude session ID. Retry paired teardown only after the durable state is safe; run Amp worker teardown last. If Amp teardown itself fails and the worker must resume, explicitly run `lifecycle worker-teardown-release` only after every registered pair still validates as safely parked. Never release the fence automatically or merely to bypass a blocker.

## Explicit legacy registration and indeterminate worker detach

Use this branch only after an owner explicitly requests recovery, supplies the exact private store path and immutable origin, and the coordinator supplies terminal Amp work authorization. Never search for a store or derive ownership. Register a pre-registry store once:

```sh
python3 "$HELPER" lifecycle register-legacy-store --origin-thread <thread-id> --store-path <exact-private-store>
```

Stop on any bounded registration blocker. Do not chmod, relink, copy, normalize, repair, or rewrite evidence to make validation pass. Exact `duplicate` is success. Registration does not detach, resolve, acquire, launch, park, or authorize teardown.

For one registered modern launch-indeterminate receipt, submit this owner-only JSON on stdin to `lifecycle detach-indeterminate-worker`:

```json
{"delegation_id":"<exact-private-id>","event_id":"<stable-detach-operation-id>","origin_thread":"<exact-thread-id>","authorization":{"terminal_state":"merged","report_sha256":"<sha256>","coordinator_authorization_sha256":"<sha256>"}}
```

Only when the registered receipt's launch intent is the exact historical pre-identity shape defined by the contract, add the explicit compatibility selector; never add missing fields to the receipt or use this selector for modern or mixed evidence:

```json
{"delegation_id":"<exact-private-id>","event_id":"<stable-detach-operation-id>","origin_thread":"<exact-thread-id>","compatibility":"pre_identity_launch_intent_v1","authorization":{"terminal_state":"merged","report_sha256":"<sha256>","coordinator_authorization_sha256":"<sha256>"}}
```

The two authorization digests are operator-supplied trusted references, not authority independently fetched or verified by the helper. Confirm that trust boundary before invoking the command; the helper only shape-validates and binds the references to the stable detach event. Use the same event ID and exact request after interruption; `duplicate` is success and conflicting reuse blocks. `matching_live_process`, `launch_identity_ambiguous_or_mismatched`, `matching_legacy_target`, `matching_legacy_session`, `process_inspection_ambiguous`, `process_inspection_unavailable`, `tmux_inspection_ambiguous`, `tmux_inspection_unavailable`, `launch_transport_active_or_indeterminate`, or any generic proof blocker preserves the receipt and durable origin fence. The legacy selector requires absence of both the exact target and exact historical session and inspection of every still-live same-owner process; it never reconstructs executable or argv identity. The launch gate serializes transport execution with absence proof and revocation commit, but detach tries it only for a short bounded interval while holding lifecycle and receipt locks; a busy gate blocks promptly and must be retried later, never waited out in place. Retained transport is authorized only for the exact pre-completion launch-indeterminate state and is revoked by completion, parking, an origin fence, or detach. Final transport authorization and `execve` are one lifecycle-then-receipt locked ordering boundary, so a concurrent fence writer commits either before authorization or after execution starts. The gate descriptor is close-on-exec: failed or not-yet-committed transport retains the gate and remains fail-closed, while a successful exec releases it instead of transferring lock ownership to the launched Claude process. Do not bypass the gate blocker or bounded subprocess inventory. After success the receipt is sealed: all launch, route, report, input, delivery, notification, park, recovery, and failure mutation routes reject, leaving only inspection, exact detach replay, and paired teardown. Do not kill a pane, inject input, retry launch, acquire a session, park, clean artifacts, or release the fence. After `detached`, rerun paired `worker-teardown --dry-run` and require the hashed pair to report `state:worker_detached`, `action:none`, and overall `ready`; then run the separate Amp worker dry-run. This authorizes only later Amp worker teardown, not receipt resolution or cleanup.

## Explicit live report-bearing pair retirement

Use this separate branch only for a validated modern `launch_intent + valid_report` receipt when terminal Amp work and trusted coordinator authorization are explicit and the exact intent-bound Claude target is still live. Never use absence detach, acquisition, acknowledgement, or ordinary parking to approximate this state. Submit the exact owner-private identity and stable operation on stdin:

```sh
python3 "$HELPER" lifecycle retire-live-indeterminate-pair <<'JSON'
{"delegation_id":"<exact-private-id>","event_id":"<stable-retirement-operation-id>","origin_thread":"<exact-thread-id>","authorization":{"terminal_state":"merged","report_sha256":"<canonical-validated-report-sha256>","coordinator_authorization_sha256":"<sha256>"}}
JSON
```

For an otherwise exact Darwin modern read-only launch intent whose sole absent field is the historical `expected_launcher_argv0_digest`, add `"compatibility":"historical_modern_read_only_launch_intent_v1"` to that request. Do not supplement or reconstruct the missing digest. The selector is invalid on Linux, for a current-modern intent, `pre_identity_launch_intent_v1`, a mixed schema, or any shape with another missing or extra field. It preserves validation of the immutable `expected_argv_digest` and every other live-retirement identity requirement, including the delegation-derived exact private `verified-claude` route and object at initial inspection and again on the retained connection immediately before stop; the supplied terminal report and coordinator-authorization digests remain the same operator-trusted references described above.

Any privacy-safe blocker preserves the durable fence, report, receipt, runtime, packet, worktree, branch, artifacts, and process. Do not search for another target or manually kill a pane/PID. A validated semantic report proves only exact candidate shape, binding, and digest, not submitting-process provenance. If the gate is busy, the holder may be a legacy post-exec target or a pre-exec transport; no lock-holder attribution is required because lifecycle and receipt locks plus the origin fence exclude the latter from final authorization. Retirement may proceed past gate contention only to the mandatory complete live-target inspection. That inspection must prove the exact intent-bound process is post-exec before durable intent or stop; failure leaves the receipt unchanged and the fence retained. Indeterminate, failed, malformed, mixed, or unvalidated evidence still blocks through its existing gate or candidate validation. The final snapshot, process verification, and stop must remain on one retained control-mode connection to the original tmux server; connection or server loss blocks, and a replacement server must never be consulted for the durable pane ID. Control responses preserve ordinary bytes, require matching complete frame tuples, and carry identity fields only through tmux's reversible `q` command-argument quoting after explicit LF/CR substitution. Each snapshot and the exact kill acknowledgement require a fresh unpredictable 256-bit correlation token; malformed framing, encoded newlines masquerading as control markers, queued frames, empty acknowledgements, wrong tokens, or ambiguous decoding blocks before terminal proof. If interruption leaves `retirement_intent` without `pair_retired`, inspect the existing evidence and replay the identical request with only `"recover":true` added. Recovery confirms exact absence first and otherwise revalidates the complete durable identity before the one exact stop; it never blindly kills again. The separately stored process-start incarnation makes a same-PID/same-start executable or argv change a blocker rather than absence. Conflicting event ID, authorization, report bytes, target identity, or receipt state blocks. After success require paired `worker-teardown --dry-run` to return the hashed pair as `state:pair_retired`, `action:none` and overall `ready`. This authorizes only the separate Amp worker teardown; it does not resolve the Claude evidence, permit cleanup, release the origin fence, or authorize merge/release/tag/worktree mutation.

## Explicit live acquired/no-report pair retirement

Use this branch only after an owner explicitly authorizes retirement of one read-only receipt whose complete event chain is exactly `created → launch_intent → launch_completed → session_acquired`, whose materialized acquired identity equals both launch completion and acquisition byte-for-byte, and which has no Claude report or input. This includes an exact-model session blocked by a provider entitlement, credit, quota, or availability prompt before semantic submission: CLI `--model` support never proves those provider properties. Do not select any offered fallback or press any provider UI choice. The terminal Amp-work report digest below is a trusted reference to the terminal Amp work; it is not a Claude semantic report and the helper never writes one. Submit the exact owner-private identity and stable operation on stdin:

```sh
python3 "$HELPER" lifecycle retire-live-acquired-no-report-pair <<'JSON'
{"delegation_id":"<exact-private-id>","event_id":"<stable-acquired-retirement-operation-id>","origin_thread":"<exact-thread-id>","authorization":{"terminal_state":"merged","report_sha256":"<terminal-amp-work-report-sha256>","coordinator_authorization_sha256":"<sha256>"}}
JSON
```

Do not use this route when any report, input, delivery, acknowledgement, parking, detach, or prior retirement event exists, or to compensate for malformed or unavailable evidence. For the exact historical-modern Darwin launch-intent shape only, the existing explicit `"compatibility":"historical_modern_read_only_launch_intent_v1"` selector remains available with all of its original constraints. It never reconstructs the missing launcher-argv0 digest and cannot substitute for a current, mixed, Linux, or otherwise incomplete identity.

For an exact pre-identity chain whose launch completion contains only tmux identity and whose later acquired/materialized identity predates normalized argv and executable path/object identity, add `"compatibility":"pre_identity_acquired_no_report_v1"` to the request. This is an explicit permanent-terminal policy query, not retirement: a same-PID/same-start executable substitution cannot be excluded from the durable historical evidence, so this route cannot safely stop a live target. This version intentionally performs no absence inspection; a future separately designed absence-only route based on pane plus PID/start disappearance is outside this contract and is not authorized by this selector. The helper validates the complete narrow shape using only durable bytes, retains the origin fence, performs no live inspection or stop, leaves the receipt byte-identical, and returns `outcome:blocked`, `blocker:pre_identity_acquired_pair_permanently_non_retirable`, `policy:preserve_receipt_runtime_artifacts_and_origin_fence`, and `remediation:paired_worker_teardown_prohibited`. Preserve the process, receipt, runtime, packet, artifacts, worktree, branch, and fence. Do not add `"recover":true`, retry with another selector, manually terminate the target, rewrite evidence, or run paired/Amp worker teardown. A fresh paired dry-run deterministically reports the same blocker even when the historical workdir is no longer present. The only actionable policy for this compatibility version is continued preservation; it does not make this historical pair eligible for retirement, parking, cleanup, or worker teardown.

### Explicit owner-authorized exact-pane disposal

Only when the owner separately authorizes destruction of the exact dedicated tmux pane/process incarnation despite unproved executable identity, use the distinct command below. This is not a retry or reinterpretation of the permanent-preservation query. The owner authorization digest is an operator-supplied trusted reference; the helper shape-validates and durably binds it but does not independently discover owner consent.

```sh
python3 "$HELPER" lifecycle dispose-exact-pre-identity-acquired-pair <<'JSON'
{"delegation_id":"<exact-private-id>","event_id":"<stable-exact-pane-disposal-id>","origin_thread":"<exact-thread-id>","compatibility":"pre_identity_acquired_no_report_v1","owner_authorization":{"decision":"dispose_exact_pane_process_incarnation","executable_identity_acknowledgement":"unproved","authorization_sha256":"<sha256>"},"authorization":{"terminal_state":"merged","report_sha256":"<terminal-amp-work-report-sha256>","coordinator_authorization_sha256":"<sha256>"}}
JSON
```

The command accepts only the complete narrow four-event receipt and exact immutable origin. Before mutation it retains one connection to the original tmux server, binds the exact session/window IDs, pane, PID/start incarnation, workdir, current command, process-command digest, launch-command digest, and Claude session from durable evidence, appends `exact_pane_disposal_intent`, then revalidates byte-for-byte and stops only that pane. It does not inspect or assert executable path/object identity or normalized argv as destruction authority. The same connection must prove the pane and PID/start incarnation absent; connection loss or a replacement server blocks. Pane/window/PID reuse, process-start or command drift, malformed/mixed/report-bearing evidence, wrong authority, and uncertain inspection fail closed.

If interrupted after intent, replay the identical request with only `"recover":true` added. Recovery proves absence first and never kills blindly. A still-live target may be stopped only after equality with both durable acquired evidence and intent; otherwise preserve it. `exact_pane_disposed` is terminal proof only for paired Amp-worker teardown: require `state:exact_pane_disposed`, `action:none`. Keep the receipt, packet, runtime metadata, artifacts, group history, worktree, branch, and origin fence. Do not manufacture Claude completion, report, input, delivery, acknowledgement, parking, or cleanup eligibility.

### Acquired/no-report executable-identity retirement execution and recovery

The operation first retains the durable origin fence and validates the exact `launch_completed → session_acquired` chain and byte-identical materialized identity. That chain proves the acquired target passed `exec`: final pre-exec transport authorization accepts only a pre-completion receipt under the same lifecycle and receipt locks. The route attempts the gate normally. A busy holder may be that pre-v0.2.26 target retaining its descriptor after successful exec or a pre-exec transport waiting before final authorization; retirement does not attribute it. While retirement holds lifecycle and receipt locks, a transport already past final authorization cannot coexist and a waiting transport later observes the fence and post-exec receipt, so exact lock-holder attribution is unnecessary. The route then verifies one exact live target against both the immutable launch intent and every byte of the completed/acquired identity, appends `acquired_retirement_intent` before process mutation, repeats complete identity validation, stops through the retained original tmux server connection, and requires bounded proof that the exact pane and process incarnation are absent. Wrong or replaced pane, pane/PID reuse, executable path/object/argv/session/workdir mismatch, receipt mutation, pending input, unavailable or ambiguous inspection, uncertain process absence, or any lifecycle race blocks without a wrong-process stop or terminal proof. Malformed, mixed, failed, indeterminate, or pre-completion evidence never reaches this exception and preserves the ordinary busy-gate blocker.

If interruption leaves `acquired_retirement_intent` without `acquired_pair_retired`, inspect the preserved evidence and replay the same request with only `"recover":true` added. Recovery proves exact absence before considering another stop; if still live, the current complete identity must equal both the acquired identity and durable retirement identity byte-for-byte. Never retry blindly. Success appends only `acquired_pair_retired`, seals every semantic and lifecycle mutation route, preserves all evidence and the fence, and grants no cleanup eligibility. Require paired `worker-teardown --dry-run` to return `state:acquired_pair_retired`, `action:none` and overall `ready`. This permits only the separate Amp worker teardown; it does not claim Claude analysis, report, input, delivery, acknowledgement, parking, semantic completion, cleanup, merge, release, tag, installation, or worktree mutation.

## Capacity or usage interruption

Record the factual `unavailable` or `untested` capability and pause. Capacity diagnostics are implementation inputs, not permission to select Claude autonomously, exceed a reserve, or claim a useful result.
