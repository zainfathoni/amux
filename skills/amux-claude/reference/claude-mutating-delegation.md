...
Run `diagnose`, then load provisional owner-only machine reserve-floor configuration for the five-hour, weekly, and every applicable model-specific window. Do not invent defaults. Submit the bounded diagnostic capacity object, all configured floors, exact provider/model/workflow/delegation/task identity, and the pool, charge-route, and admitted-impact evidence classifications with `acknowledged_unknown_capacity:false` to `capacity decide-mutating`. Stage A accepts only `unknown` or `unsupported` for those three evidence classifications and therefore never returns autonomous admission. The currently documented CodexBar usage payload has no source-contract or schema-version discriminator, so diagnostics classify it as unavailable and never manufacture those fields.

Every available window must have a floor, and every configured model-specific floor must have exactly one correctly classified bounded window. A reset is reliable only while it is strictly in the future and no later than one declared window duration from the decision's single UTC observation; every plan and execute re-decision checks this again. Remaining capacity is `100 - used_percent`; the smallest `remaining - floor` margin is the governing window. Any known negative margin blocks launch as a hard reserve violation even when provenance or another window is missing, unsupported, or low-confidence. Missing configured floors are also non-overridable. Unknown evidence returns `acknowledgement_required`, disables autonomous selection, and includes a privacy-safe `owner_action` containing only the exact prior decision digest, acknowledgement flag, and five-minute expiry. Apply that generated action to the unchanged request; do not construct or copy private capacity JSON. The digest binds provider, exact model, workflow/delegation/task, observation and provenance, floors, and all three evidence classifications. Immediately before intent the helper re-decides expiry and evidence, then consumes the action once while holding the canonical lifecycle lock and checking every registered receipt store. Changed, expired, concurrently consumed, or already consumed actions fail closed, and the resulting decision remains `autonomous_selection:false`. Bind the returned decision digest to the receipt.

Completion: the decision says `may_proceed:true`, every known available window remains at or above its floor, and one fresh exact owner acknowledgement—not autonomous inference—accepted the unknown Stage A impact.
...
Create an owner-only self-contained packet that states the authority matrix, baseline, branch, one-commit or zero-commit-blocked outcomes, submission freeze, normal hook policy, privacy exclusions, and prohibited Git/GitHub/lifecycle actions. Use only approved public repository sources and independently safe summaries. Call `launch plan` with `workflow:mutating`, `model:claude-opus-4-8`, the original capacity observation/floors/generated acknowledgement action, and the exact preparation fields. The helper recomputes and returns the capacity decision; callers cannot supply a preconstructed launch decision. The mutating policy is distinct from the read-only thinker policy, binds the exact model into policy and argv digests, and must report `writer_authority:exclusive`.
...
Completion: one receipt-bound fresh `claude-opus-4-8` incarnation owns only the declared worktree edits; Amp remains sole integration and lifecycle authority. Authentication, entitlement, quota, credit, and model-availability blockers before semantic submission preserve the acquired/no-report evidence. They authorize no provider input, model change, fallback, or relaunch.
