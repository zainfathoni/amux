...
- Pi 0.82.1 may create `models-store.json` as ordinary provider-catalog cache state. The helper accepts a bounded regular cache for unrelated providers but rejects an `openai-codex` entry because a newer cached same-ID model can replace Spark's built-in request metadata; it rechecks this admission after execution.
