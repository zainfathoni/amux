...
| `Run Pi on Spark on a physical runner in a dedicated local worktree/tmux with no Orb` | [`../experimental/pi-spark-local`](../experimental/pi-spark-local) | Explicit owner authorization only; selects the existing physical-host helper, never the Orb recipe; exact Pi 0.82.1, exact Node `>=22.19.0` engine contract, and Spark model; one attempt with no fallback; one tracked-file replacement in a clean dedicated worktree, not arbitrary local work. |
| `Spike one bounded local file replacement` | [`../experimental/pi-spark-local`](../experimental/pi-spark-local) | Explicit owner authorization only; exact Pi 0.82.1, exact Node `>=22.19.0` engine contract, and Spark model; one tracked file; clean worktree; open-ended results untrusted. |
