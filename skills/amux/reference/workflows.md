...
A launch-indeterminate pair remains a blocker unless the explicit owner-authorized branch in [`claude-delegation-recovery.md`](claude-delegation-recovery.md#explicit-legacy-registration-and-indeterminate-worker-detach) has appended a complete `worker_detached` proof. Legacy registration and detach are skill-owned recovery operations, not automatic teardown steps and not generic amux resources. A detached pair remains unresolved and fenced; paired teardown may only return `action:none` for that pair and permit teardown of the Amp worker while preserving every receipt and artifact.
