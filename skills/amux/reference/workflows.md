...
Use `--mode low`, `--mode medium`, `--mode high`, or `--mode ultra` for Amp's built-in Dial modes; `medium` is the default recommendation for fuzzy issue work. `amux` still passes arbitrary custom/plugin mode strings through to Amp, so do not treat the built-in list as validation.
