...
Every skill-driven spawn MUST pass `--mode medium` unless the user explicitly requests another mode. Substitute the exact requested built-in or plugin mode; never infer `high` or `ultra` from complexity, size, urgency, or duration. Reuse the same idempotency key only for the identical request. If creation becomes indeterminate, inspect the operation and external state; never change the key or resubmit blindly. After read-only export proves the complete assignment is in the exact provisioned thread, append `--reconcile` to the complete identical request. This narrow path verifies immutable request identity and exact delivery, then creates or verifies the worker row and local tmux client without thread creation, resubmission, or alternate-thread adoption. Other indeterminate outcomes remain terminal.
