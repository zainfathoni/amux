...
The commands below this point operate the legacy per-workdir registry and tmux lifecycle. They remain available during migration, but should not be used for new runner topology when one native profile suffices. The narrow exception is an owner-requested primary-service rollout recovery companion: `<host>-maintenance` as workspace and runner ID, the canonical Amux checkout as workdir, a normally parked binding launched as a bare detached tmux runner only when needed, and no native profile, service, or automatic login launch. Verify exact row, cwd, argv, and independent process group before replacement, then park it after verifying the primary; this channel does not promise reboot or tmux-server survival.
...
For native profiles, systemd or launchd runs the resolved Amp executable directly and keeps it alive. launchd starts at GUI login; a systemd user service starts with the user's service manager and requires externally configured lingering to start without login. Native Amp owns update and idle-restart behavior, subject to its installation and settings. Self-owned legacy maintenance cannot be installed or run while native services are installed or activation pending; external or package-manager ownership remains an explicit compatibility-tail option. The old bare `amux`/`amux launch --all` activation applies only to legacy per-workdir runners and must be disabled before parking them for native cutover.
