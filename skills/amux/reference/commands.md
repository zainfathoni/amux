...
| `spawn` | persist exact retained ownership; optional group intent then additive label ensure; neither implies task execution | reject overlapping amux Runner workdir | create one exact worker pane; attempt one literal paste, then Enter only after paste success; delivery remains unacknowledged | create one empty local thread in canonical physical cwd; return retained-indeterminate |
...
- Prefer native create → adopt when the repository is an Amp Workspace Project. For the projectless physical-host exception, executor selection first routes to the intended host; local `amux spawn` (exact alias: `amux worker spawn`) requires a canonical existing cwd, workspace, window, optional one existing group, exact mode (default `medium`), and a bounded prompt from `--prompt-file <path|->`. It reads the prompt before taking the mutation lock, creates once, attempts one paste, and attempts Enter only after paste success. Neither successful nor failed input commands are retried. With no supported delivery acknowledgement, it persists exact worker ownership as `retained_indeterminate` plus group/label ownership and returns exit `1`, reporting exact thread/window/pane, input result, and completed persistence phases. Input-command results, persistence, and a live local client never establish assignment delivery or task execution; list/doctor report assignment state separately. Never automatically retry, repaste, submit, clean up, archive, search, reconcile, or use an alternate receiver; owner-directed authenticated follow-up is outside spawn. Unparseable creation results are indeterminate and do not echo raw output. It does not require a local process `--runner-id` alias or read pane/transcript/thread/preview state. Legacy spawn operation records are read-only evidence and must not be retried or reconciled.
