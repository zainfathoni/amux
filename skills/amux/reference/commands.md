...
- Group IDs are at most 32 characters and match `^[a-z0-9]+(?:-[a-z0-9]+)*$` byte-for-byte. Local `groups.tsv` intent is authoritative and survives worker teardown and finish. External labels are add-only: removal is local-only and reports `external_sync: unsupported` plus `drift: may_remain_indefinitely`.
