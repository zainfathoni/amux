...
| `spawn` | persist exact worker; optional group intent then additive label ensure after input attempts | reject overlapping amux Runner workdir | create one exact worker pane; attempt one literal paste, then Enter only after paste success | create one empty local thread in canonical physical cwd |
...
- Prefer native create → adopt when the repository is an Amp Workspace Project. For the projectless physical-host exception, executor selection first routes to the intended host; local `amux spawn` (exact alias: `amux worker spawn`) requires a canonical existing cwd, workspace, window, optional one existing group, exact mode (default `medium`), and a bounded prompt from `--prompt-file <path|->`. It reads the prompt before taking the mutation lock, creates once, attempts one paste, attempts Enter only after paste success, reports each completed persistence/group phase, and never retries or cleans up post-create state. Unparseable creation results are indeterminate and do not echo raw output. It does not require a local process `--runner-id` alias. Legacy spawn operation records are read-only evidence and must not be retried or reconciled.
