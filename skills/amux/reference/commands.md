...
| `remove` | remove selected config; remove worker shelf intent | currently retain and reject until exact process/native-catalog absence is provable | stop verified worker clients only; runner removal does not stop | none |
...
| `reconcile` | synchronize shelf/remote drift; remove only proven-missing safe worker bindings | present-directory no-op; missing configured rows retained until positive process/catalog absence is provable | only verified worker repairs | worker archive synchronization only for present bindings |

Worker `remove` differs from `unpin`: it stops the selected verified local client and deletes configuration, while unpin never stops it. Runner `remove`, `unpin`, and the row-deleting missing-workdir `reconcile` branch currently fail closed and retain the row because current Amp APIs cannot bind the native catalog entry to the exact local process and prove positive process/catalog absence; use `runner park` to stop an exact owned process while retaining its row. Dry-run fails at the same preflight. Worker unpin removes the worker row and matching shelf intent. Neither mode changes remote thread state. Worker `remove` never archives. `teardown` is worker-only remove plus verified remote archival.

Worker reconcile accepts canonical `--workdir` scope. `workers.tsv` is the sole thread↔workdir authority: a directory without a binding is reported and preserved, while a binding is removed only when its canonical directory is proven missing, local runtime is absent, and no blocked never-authorized report remains. It never derives identity or workdir from `report_id` or `groups.tsv`; unreadable, non-directory, relative, read-error, and unknown states fail closed. Runner pin requires a canonical existing directory. Git repository, worktree, and lock state are not runner requirements. Runner reconcile never adopts or deletes ambiguous Amp-owned processes and currently retains missing-directory rows pending authoritative process/catalog absence evidence.
