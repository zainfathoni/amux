...
Before launch, `launch plan` must verify a supported exact-process-identity implementation, tmux, the installed Claude flags, exact canonical repository, base commit, clean dedicated linked worktree without optional Git locks, private packet mode, and deterministic packet, launch-policy, and launch-command digests. Darwin uses its existing kernel process APIs. Linux read-only launch requires stable `/proc/<pid>/stat` start time, NUL-delimited `/proc/<pid>/cmdline`, and `/proc/<pid>/exe` target/device/inode identity; those sources are read twice and must agree. Missing, denied, changed, recycled, truncated, or ambiguous process data fails closed. Linux mutating launch remains unavailable. The plan is a dry run: it writes no receipt, runtime configuration, tmux window, or Git index metadata. `launch execute` requires an already-created receipt whose immutable digests match and a stable event ID. It durably reserves launch intent before writing owner-only MCP/settings files and launching one detached tmux window. A completed exact replay returns the original identity without relaunching; an intent lacking a result is indeterminate and forbids automatic relaunch.
