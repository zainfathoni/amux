...
The permission adapter is observation-only for every current native tool. Configure it only to collect bounded capability results; its exit is always allow. Only the explicit automatic-spawn resolver call can return binding `reject`. Never bypass a binding `ask` or `reject` from a future independently promoted tuple.
