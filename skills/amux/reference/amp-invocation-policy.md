...
The legacy generalized automatic `/amux spawn` policy tuple remains retired. Issue #297 restores only the explicit projectless physical-host route: one exact live local native runner ID, its canonical cwd, and a bounded prompt, with no executor fallback. Preserve the owner's exact mode instruction; use `medium` when the owner did not name another mode. The resolver remains only for compatibility with installed historical skill material and does not broaden or authorize inputs outside that narrow CLI contract.
