...
Exit `1` means mutation may have started. Inspect `successful`, `skipped`, and `failed`, then inspect only state authorized by the command-specific contract before any deliberate owner action. Exit `2` means request/preflight rejection before mutation. A projectless physical-host spawn always returns exit `1` after its bounded input attempt because no supported delivery acknowledgement is available. Its `retained-indeterminate` outcome prints the exact thread/window/pane, assignment state, paste/Enter result, and every completed worker/group/label persistence phase. Human output records exact worker persistence before group/label work; JSON uses `report-delivery-indeterminate`. The durable worker assignment state remains `retained_indeterminate` even when the local client is live. Delivery is indeterminate, acknowledgement is unavailable, and delivery/execution are unproven. Preserve all of it and stop. The composer may contain unsent prompt bytes. Never automatically rerun, repaste, press Enter, submit, archive, kill, clean up, search, inspect pane/transcript/thread/preview state, reconcile, or use an alternate receiver. Owner-directed authenticated follow-up is manual recovery outside spawn. Never retry, reconcile, convert, delete, or mark successful a preserved legacy spawn operation record.
...
## Experimental Tycho report inbox recovery

The skill-owned Tycho adapter is separate from stable group/report/callback state. Follow [`tycho-report-bridge.md`](tycho-report-bridge.md): recover `valid_report` from its owner-private store, explicitly consume it as the exact bound Amp origin to establish `delivered`, then acknowledge separately. A successful, failed, stale, missing, or indeterminate tmux notification is never delivery and is never automatically resent. There is no watcher, polling-as-delivery, Tycho process lifecycle ownership, group membership, finish authority, or arbitrary Amp Web-thread delivery route.
