...
Exit `1` means mutation may have started. Inspect `successful`, `skipped`, and `failed`, then inspect config, tmux, Git worktrees, and remote thread state before retrying. Exit `2` means request/preflight rejection before mutation. Never change an indeterminate spawn's stable idempotency key to force a duplicate creation. For the specific timeout reporting that the assignment was not found in the provisioned thread or one fresh receiver, first prove through read-only export that the complete assignment is in the exact provisioned thread. Then rerun the complete identical spawn request with the same key and `--reconcile`. Reconciliation verifies the immutable request and exact assignment, creates or verifies the worker row and local tmux client, and completes the original operation without thread creation, message resubmission, or alternate-thread adoption. Other indeterminate outcomes remain terminal.
