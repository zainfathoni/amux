...
The operation first retains the durable origin fence and verifies one exact live target against both the immutable launch intent and every byte of the completed/acquired identity. It then appends `acquired_retirement_intent` before process mutation, repeats complete identity validation, stops through the retained original tmux server connection, and requires bounded proof that the exact pane and process incarnation are absent. Wrong or replaced pane, pane/PID reuse, executable path/object/argv/session/workdir mismatch, receipt mutation, pending input, unavailable or ambiguous inspection, uncertain process absence, a busy launch transport, or any lifecycle race blocks without a wrong-process stop or terminal proof.
