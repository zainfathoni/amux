...
| `/amux health <workspace>` | Load [`workflows.md#health-check-interactive-workers-before-replacement`](workflows.md#health-check-interactive-workers-before-replacement), then perform the skill-only health workflow. | Inspect amux/tmux state, ping only verified Amp panes with unique tokens, classify `healthy` / `no-live-window` / `no-response` / `mismatched` / `ambiguous` / `shelved`, and report before any replacement; do not archive, unpin, park, kill, or spawn. | [`workflows.md#health-check-interactive-workers-before-replacement`](workflows.md#health-check-interactive-workers-before-replacement) |
