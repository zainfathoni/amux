...
| `Run a bounded local Pi Spark microtask` | [`pi-spark-local-executor.md`](pi-spark-local-executor.md) | Explicit-only Darwin persistent-host route; no-tool bounded replacements, exact Spark provenance, reusable OAuth preservation, and trusted before/after billing evidence. |
