...
- Legacy runner pin/list/launch/doctor/park/restart/teardown/remove and minimum fail-closed reconcile remain active during migration. Do not create new per-workdir bindings when one native multi-directory profile suffices, except for the owner-requested primary-service rollout recovery companion below. Before native cutover, disable login automation that invokes bare `amux` or `amux launch --all`, remove self-owned maintenance (external ownership is an explicit tail option), then park → soak through login or reboot → unpin. Preserve the worktree; never teardown for cutover.
- A primary-service rollout recovery companion uses `<host>-maintenance` as both workspace and runner ID, the canonical Amux checkout as workdir, and the bare legacy tmux launch with no discovery, explicit-directory, or remote-terminal flags. Keep its row but park its process when no rollout or recovery is active. It must not appear in `native-runners.json`, `runner-services.json`, an OS service, or login automation. Before relying on it, inspect the exact row, dry-run launch, and verify detached tmux state, exact cwd/argv, and a process group independent from the primary. Keep it live through primary replacement, verify both runners afterward, then dry-run and park it again. It survives primary-service replacement only while launched. Migrate an obsolete absent row only with supported dry-run-first unpin → pin; never edit registry, tmux, or PID state manually.
...
- **Prepare a primary-runner rollout recovery channel**: inspect or create the exact parked `<host>-maintenance` legacy binding at the canonical Amux checkout, dry-run then launch it, and verify independent tmux/process identity before replacing the primary service. Afterward, verify the primary, then dry-run and park the companion. Never model it as a native profile, OS service, or automatic login launch.
