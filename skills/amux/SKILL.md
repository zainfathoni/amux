...
- A primary-service rollout recovery companion uses `<host>-maintenance` as both workspace and runner ID, the canonical Amux checkout as workdir, and the bare legacy tmux launch with no discovery, explicit-directory, or remote-terminal flags. It must not appear in `native-runners.json`, `runner-services.json`, or an OS service. Before relying on it, inspect the exact row, dry-run launch, and verify detached tmux state, exact cwd/argv, and a process group independent from the primary. Keep it live through primary replacement and verify both runners afterward. It survives primary-service replacement, not reboot or tmux loss. Migrate an obsolete absent row only with supported dry-run-first unpin → pin; never edit registry, tmux, or PID state manually.
...
- **Prepare a primary-runner rollout recovery channel**: inspect or create the exact `<host>-maintenance` legacy binding at the canonical Amux checkout, dry-run then launch it, and verify independent tmux/process identity before replacing the primary service. Never model it as a native profile or OS service.
