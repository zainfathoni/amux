...
- **Spawn a worker for ...**: load [`reference/workflows.md`](reference/workflows.md); create directly on the executor/workdir intended for the work, then adopt locally—adoption neither migrates execution nor verifies continued affinity. Use explicit executor and `medium`. The task-only initial message includes the **absolute path to the loaded skill's** `reference/contract-v1.md` (never a bare relative path). `amux spawn` and `amux worker spawn` are removed tombstones.
