...
- **Recover indeterminate Claude worker evidence**: only after explicit owner recovery authorization, load [`reference/claude-delegation-recovery.md`](reference/claude-delegation-recovery.md#explicit-legacy-registration-and-indeterminate-worker-detach). Register only an exact supplied owner-private store and detach only with terminal coordinator evidence plus bounded exact-target absence. Preserve the unresolved receipt, artifacts, worktree, immutable origin, and durable origin fence; never infer, adopt, retry, acquire, input, kill, park, or clean.
