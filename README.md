...
An exact `#<issue>` title prefix owns issue identity. The window must be an issue-unprefixed semantic slug; obvious duplicates such as `issue-110-install-diagnostics` are rejected before side effects. `--message`, `--message-file`, and `--message-stdin` are mutually exclusive. Spawn requires a stable idempotency key. If verification times out after delivery starts, first prove through read-only export that the complete assignment is in the exact provisioned thread, then rerun the complete identical spawn request with `--reconcile`. amux verifies the immutable request hash and exact provisioned assignment, creates or verifies the worker row and local tmux client, and transitions the original operation without creating a thread or resubmitting the message. Other indeterminate outcomes and alternate-thread adoption during reconciliation are rejected.
