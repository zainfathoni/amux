...
For a proven pre-cutover group/report drain, the already-bound worker uses one stable report ID for `blocked`, `ready`, and terminal `merged`. `ready` means implementation, tests, one review, PR, and normal CI are complete. A callback token only wakes the coordinator. The coordinator acknowledges receipt separately, independently verifies PR URL/head/scope/mergeability/closing issue, worktree and CI, merges only with separate authority, verifies post-merge CI, and records durable finish authorization before `/amux finish` begins. Finish then runs every knowable read-only check before mutation, presents one exact summary for one owner approval, parks the exact verified live worker, and records only that already-authorized merged-PR report's terminal transition—never authorization or a new report lifecycle for cleanup. A terminal report persisted under its exact immutable binding remains successful when only its separate best-effort callback failed; finish verifies history rather than adding an acknowledgement round trip.

The ordinary finish path also admits an explicitly completed review-only worker without fabricating a PR, merge, or terminal report state. In either completion case it requires one exact present, index-only, unshared, runner-free attached worktree and complete host-process evidence; keeps the local branch so unique or unpushed commits remain reachable; unlocks only when Git reports the exact worktree locked; removes it normally without force; and runs exact-thread `amux teardown` last to archive the completed thread. Local/remote branch deletion is never implicit, and group/report/provider evidence survives finish.

All lifecycle mutations share one lock. Exit `2` contention writes nothing and requires waiting for the current operation before retrying the identical operation/report ID. Stale/recycled/missing callback leases fail closed and are repaired only by explicit registration. Before finish, callback failure leaves the durable report pending and the worker alive. During finish's already-authorized terminal transition, exact terminal history is sufficient when only the separate callback failed. Never retry notification into a suspected busy composer: recover from durable pending/history state and acknowledge it directly when acknowledgement is still part of the pre-finish stage. Duplicate/reordered wake-ups and coordinator restarts are likewise recovered from durable state, never inferred tmux delivery. Do not force-delete branches, auto-release, infer finish from a late callback, or repeatedly read unrelated Amp threads.
