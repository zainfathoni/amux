...
The bundled `/amux` skill provides the coordinator procedure. Worker assignments stay task-only and point at `reference/contract-v1.md` once. In summary: inspect native dependencies and active PR/branch/worktree/API overlap; fetch and create dedicated worktrees from fresh `origin/main`; use semantic issue-unprefixed windows and explicit `--mode medium` unless overridden; declare the group and register the exact verified coordinator pane; then spawn with `--group` so membership binds only after authoritative alternate-thread adoption.
