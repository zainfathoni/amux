...
Every task Lead thread title begins with `🎖️ `; member worker titles do not. Set the Lead title explicitly with `amp threads rename <lead-thread> "🎖️ <task-title>"` after obtaining its exact identity. This is presentation only and is independent of executor identity: it implies neither placement nor authoritative group role.
