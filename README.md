...
Executor selection routes this command to the intended physical host; `amux spawn` itself is local by construction and canonicalizes the requested workdir. It reads and bounds the prompt before taking the machine mutation lock, runs `amp threads new --mode ...` once in that cwd, creates one exact tmux continue window, attempts one literal prompt paste, and only after a successful paste attempts one Enter. The tmux commands acknowledge only a local input attempt, not assignment delivery. Whether paste/Enter completes or fails, the command makes no second input attempt. Because the public local Amp CLI provides no compatible delivery acknowledgement, the command persists the exact worker ownership with durable assignment state `retained_indeterminate` and, when requested, group membership and its add-only label, then always returns exit `1` with a distinct `retained-indeterminate` outcome. Those persistence phases preserve the exact thread/window/pane for deliberate owner recovery; they do **not** mean that the assignment was delivered or that the group member is executing it. Worker list/doctor and JSON preserve the assignment state separately from local-client process state, so a live pane is not a running-task claim. The retained outcome lists the input-attempt result and every completed persistence phase, warns that the composer may contain unsent prompt bytes, and prohibits automatic retry, repaste, submit, cleanup, archive, search, reconciliation, or alternate receivers. Owner-directed authenticated follow-up is manual recovery outside the spawn contract.
...
| `spawn` | persist exact retained ownership; optional group intent then additive label ensure; neither implies task execution | reject overlapping amux Runner workdir | create one exact worker pane; attempt one paste, then Enter only after paste success; delivery remains unacknowledged | create one empty local thread in the canonical local cwd; return retained-indeterminate |
